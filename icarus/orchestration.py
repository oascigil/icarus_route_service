...
def run_scenario(settings, params, curr_exp, n_exp):
    ...
    try:
        ...
        # Assign content names to route services
        if 'rs_placement' in tree:
            rs_spec = tree['rs_placement']
            rspl_name = rs_spec.pop('name')
            if rspl_name not in ROUTE_SERVICE_PLACEMENT:
                logger.error('No route service placement named %s was found.' % rspl_name)
                return None
                ROUTE_SERVICE_PLACEMENT[rspl_name](topology, **rs_spec)
