...
__all__ = ['uniform_content_placement', 'weighted_content_placement', 'transit_local_content_placement']
...
def transit_local_content_placement(topology, contents, percentage_local=0.2, percentage_edge=0.8, seed=None):
    ...
    for c in contents:
        if n_internal_nodes > 0:
            content_placement[random.choice(internal_sources)].add(c)
    ...
    topology.graph['edge_contents'] = set(edge_contents)
    topology.graph['internal_contents'] = set(internal_contents)
