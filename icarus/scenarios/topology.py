...
class IcnTopology(fnss.Topology):
    ...
    def rsn_nodes(self):
        return {v: self.node[v]['stack'][1]['rsn_size']
                for v in self
                if 'stack' in self.node[v]
                and 'rsn_size' in self.node[v]['stack'][1]
                }
...
def topology_rocketfuel_latency(asn, source_ratio=1.0, ext_delay=EXTERNAL_LINK_DELAY, **kwargs):
    ...
    for i in range(len(sources)):
        ...
        if routers[i] in onedeg:
            edge_sources.append(sources[i])
        else:
            internal_sources.append(sources[i])
...
#         topology.add_edge(sources[i], source_attachments[i], delay=ext_delay, type='external')
    ...
    for i in range(len(routers)):
        ...
        if routers[i] in onedeg:
            edge_receivers.append(receivers[i])
    # Set weights to latency values
    for u, v in topology.edges_iter():
        topology.edge[u][v]['weight'] = topology.edge[u][v]['delay']
    # Deploy stacks on nodes
    topology.graph['icr_candidates'] = set(routers)
    topology.graph['edge_nodes'] = set(onedeg)
    topology.graph['edge_receivers'] = set(edge_receivers)
    topology.graph['edge_sources'] = set(edge_sources)
    topology.graph['internal_sources'] = set(internal_sources)
    topology.graph['internal_receivers'] = set(receivers) - set(edge_receivers)
    topology.graph['routers'] = set(routers)
    topology.graph['internal_nodes'] = set(routers) - set(onedeg)
    for v in sources:
        fnss.add_stack(topology, v, 'source')
    for v in receivers:
        fnss.add_stack(topology, v, 'receiver')
