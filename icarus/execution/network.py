...
class NetworkView(object):
    ...
    def rs_source(self, k):
        """Return a list of node identifiers where the route for the content is found
        """
...
class NetworkModel(object):
    ...
    def __init__(self, topology, cache_policy, shortest_path=None):
        ...
        self.source_node = {}
        # Dictionary mapping content to route servers
        self.rs_source = {}
        ...
        for node in topology.nodes_iter():
            ...
            if stack_name == 'router':
                ...
                if 'route_service' in stack_props:
                    contents = stack_props['route_service']
                    for content in contents:
                        if content not in self.rs_source.keys():
                            self.rs_source[content] = [node]
                        else:
                            self.rs_source[content].append(node)
